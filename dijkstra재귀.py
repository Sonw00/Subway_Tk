...
def search_stored_stations(stored_stations):    # 저장했던 검색할 역중에 하나 반환
    ...
    while len(stored_stations) > 0:
        stored_station = stored_stations.pop(0)  # 가장 첫 번째 역을 꺼냄
        ...
        if len(next_stations) > 0:
            ...
            return visit,next_stations, route, shortestTime, stored_stations
        else :
            print(visit+"역 삭제 ")
            
    if len(stored_stations) == 0:
        return None, [], [], None, []  # 적절한 종료 값을 반환
...
def calculate_shortest_time():
    ...
    if start_station and end_station:
        ...
        def visitPlace(visit, route, shortestTime):
            print(f"\n재귀호출된 역: {visit}")
            print(f"현재까지 경로: {route}")
            stored_stations = []

            if visit == end_station:  # 도착역에 도달한 경우
                route.append(visit)
                routing.append({"route": route.copy(), "shortestTime": shortestTime})
                print(f"추가된 도착 경로: {route}")
                return

            if visit in visited:  # 이미 방문한 역이라면 탐색 중지
                print(f"{visit} 이미 방문한 역입니다. 경로를 종료합니다.")
                return

            visited.add(visit)  # 방문한 역으로 추가

            next_stations = landscape[visit].copy()
            next_stations = list(set(next_stations) - set(route))  # 이미 방문한 역 제외
            print(f"next_stations: {next_stations}, route: {route}")

            if len(next_stations) == 0:  # 더 이상 이동할 역이 없는 경우
                print(f"{visit}에서 이동할 수 있는 역이 없습니다.")
                return
            ...
            for next_station in next_stations:
                ...
                current_line = line_info[visit]

                time_addition = 6 if not set(current_line).intersection(set(next_line)) else 2  # 환승 시 6분, 같은 노선 2분 추가
                visitPlace(next_station, new_route, shortestTime + time_addition)

                # 갈림길이 발생할 경우 나머지 역을 저장
                if len(next_stations) > 1:
                    for other_station in next_stations[1:]:
                        stored_stations.append({
                            "station": other_station,
                            "route": new_route.copy(),
                            "shortestTime": shortestTime + time_addition
                        })

            # 저장된 경로 탐색
            if stored_stations:
                next_visit, next_stations, next_route, next_shortestTime, stored_stations = search_stored_stations(stored_stations)
                if next_visit is not None:
                    visitPlace(next_visit, next_route, next_shortestTime)
