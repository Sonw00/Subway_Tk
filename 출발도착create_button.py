    ...
    for station in stations:
        ...
        if abs(x - station_x) < 10 and abs(y - station_y) < 10:  # 좌표 근처 클릭 감지
            ...
            # 도착 지점 선택 (출발이 선택된 상태에서)
                ...
                text = canvas.create_text(station_x, station_y - 25, text="도착", fill="black", font=("맑은 고딕", 9))
...
def create_buttons(station_x, station_y, type):
    ...
    button = tk.Button(canvas, text="출발" if type == 'start' else "도착",
                       command=lambda: delete_button(type))
    ...
    for station in stations:
        if station['name'] == selected_value:
            if type=='start':
                ...
                if selected_station['start']:
                    delete_button(type)
                button = create_buttons(station['x'], station['y'], 'start')
                text = canvas.create_text(station['x'], station['y'] - 25, text="출발", fill="black", font=("맑은 고딕", 9))
                selected_station['start'] = {'button': button, 'text': text, 'station': station['name']}
                start_station_var.set(station['name'])  # 도착역 설정
                
            elif type=='end':
                print(f"도착 역 선택: {selected_value}")
                if selected_station['end']:
                    delete_button(type)
                
                button = create_buttons(station['x'], station['y'], 'end')
                text = canvas.create_text(station['x'], station['y'] - 25, text="도착", fill="black", font=("맑은 고딕", 9))
                selected_station['end'] = {'button': button, 'text': text, 'station': station['name']}
                end_station_var.set(station['name'])  # 도착역 설정
